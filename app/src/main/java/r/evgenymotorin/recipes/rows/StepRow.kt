...
class StepRow(private val indexNum: String, private val step: Step): Item<ViewHolder>() {
    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.itemView.description_step_row.text = step.description

        if (step.imgUrl != null) {
            viewHolder.itemView.image_container_step_row.visibility = View.VISIBLE
            Picasso.get().load(step.imgUrl).into(viewHolder.itemView.image_step_row)
        } else
